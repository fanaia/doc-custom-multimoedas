...
type Catalogs = { bases: Item[]; imagens: Item[]; templates: Item[]; documentos: Item[]; configuracoes: Item[] };
...
function BasesOmiePage() {
  ...
  return <div><Header title="Bases Omie" description="Conecte todas as empresas Omie usadas nesta operação." />
    ...
    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(280px,1fr))", gap: 14 }}>{query.data?.bases.map(base => <div style={card} key={base._id}>
      ...
      <div style={{display:"flex",gap:8,flexWrap:"wrap"}}><button style={button} onClick={() => action.mutate({id:base._id,operation:"testar"})}>Testar conexão</button><button style={secondary} onClick={() => action.mutate({id:base._id,operation:"etapas/sincronizar"})}>Sincronizar etapas</button><button style={secondary} onClick={() => action.mutate({id:base._id,operation:"categorias/sincronizar"})}>Sincronizar categorias</button><button style={secondary} onClick={() => action.mutate({id:base._id,operation:"contas-correntes/sincronizar"})}>Sincronizar contas correntes</button></div>
    </div>)}</div>
  </div>;
}
...
const replaced = new Set(["BaseOmie", "Imagem", "Template", "Configuracao"]);
const uiManifest = {
  ...ui,
...
startCentralFromManifest({ app, ui: uiManifest as Parameters<typeof startCentralFromManifest>[0]["ui"] }, {
  ...
  customComponents: { BasesOmiePage, ConfiguracoesPage, ImagensPage, TemplatesPage, DocumentosPage },
});
