...
function GatilhosPage() {
  const { http } = useOonApi(); const query = useCatalogs(); const cache = useQueryClient();
  const [gatilhoId,setGatilhoId]=useState(""); const [editingTrigger,setEditingTrigger]=useState<Item|null>(); const [editing,setEditing]=useState<Item|null>(); const [baseOmieId,setBaseOmieId]=useState(""); const [advanceEnabled,setAdvanceEnabled]=useState(false); const [message,setMessage]=useState(""); const [error,setError]=useState("");
  ...
  const accounts=(query.data?.contasCorrentes||[]).filter(item=>item.status==="ativo"&&String(item.baseOmieId?._id||item.baseOmieId)===selectedBase);
  const categories=(query.data?.categorias||[]).filter(item=>item.status==="ativo"&&String(item.baseOmieId?._id||item.baseOmieId)===selectedBase);
  const finish=(text:string)=>{setMessage(text);setError("");setEditing(undefined);setBaseOmieId("");setAdvanceEnabled(false);cache.invalidateQueries({queryKey:["doc-custom-catalogs"]});};
  ...
  const save=useMutation({mutationFn:async({id,payload}:any)=>id?(await http.put(`/api/doc-custom/gatilhos/${gatilhoId}/bases/${id}`,payload)).data:(await http.post(`/api/doc-custom/gatilhos/${gatilhoId}/bases`,payload)).data,onSuccess:data=>finish(data.message||"Etapas e adiantamento salvos com sucesso."),onError:e=>{setMessage("");setError(errorText(e));}});
  ...
  function submit(event:FormEvent<HTMLFormElement>){event.preventDefault();const payload:any=Object.fromEntries(new FormData(event.currentTarget));payload.gerarAdiantamento=advanceEnabled;save.mutate({id:editing?._id,payload});}
  ...
  function beginEdit(item:Item){setEditing(item);setBaseOmieId(String(item.baseOmieId?._id||item.baseOmieId));setAdvanceEnabled(Boolean(item.gerarAdiantamento));setMessage("");setError("");}
  const templateOptions=(tipo:string)=>query.data?.templates.filter(item=>item.tipo===tipo&&item.status==="ativo")||[];
  const refId=(value:any)=>value?._id||value||"";
  return <div><Header title="Gatilhos" description="Cadastre os documentos, etapas e regras financeiras para cada Base Omie."/><Notice message={message} error={error}/>
    ...
    {editing!==undefined&&<form onSubmit={submit} style={{...card,display:"grid",gridTemplateColumns:"repeat(2,minmax(0,1fr))",gap:12,marginBottom:18}}>
      ...
      <div style={{gridColumn:"1/-1",borderTop:"1px solid #e2e8f0",paddingTop:12}}>
        <label style={{display:"flex",gap:9,alignItems:"center",fontWeight:600}}><input type="checkbox" checked={advanceEnabled} onChange={event=>setAdvanceEnabled(event.target.checked)}/>Gerar adiantamento no Omie ao concluir</label>
        <p style={{margin:"6px 0 12px",color:"#64748b"}}>As parcelas da OS serão marcadas como adiantamento ao avançar para a etapa de sucesso.</p>
        {advanceEnabled&&<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
          <select aria-label="Conta corrente do adiantamento" style={input} name="contaCorrenteAdiantamentoId" defaultValue={refId(editing?.contaCorrenteAdiantamentoId)} required><option value="">Conta corrente do adiantamento</option>{accounts.map(item=><option key={item._id} value={item._id}>{item.descricao} ({item.codigo})</option>)}</select>
          <select aria-label="Categoria padrão do adiantamento" style={input} name="categoriaAdiantamentoId" defaultValue={refId(editing?.categoriaAdiantamentoId)} required><option value="">Categoria padrão do adiantamento</option>{categories.map(item=><option key={item._id} value={item._id}>{item.descricao} ({item.codigo})</option>)}</select>
        </div>}
      </div>
      ...
      {advanceEnabled&&(!accounts.length||!categories.length)&&<p style={{gridColumn:"1/-1",color:"#c92a2a"}}>Sincronize contas correntes e categorias desta Base Omie antes de habilitar o adiantamento.</p>}
      <div style={{gridColumn:"1/-1",display:"flex",gap:8}}><button style={button} disabled={save.isPending||stages.length<3||(advanceEnabled&&(!accounts.length||!categories.length))}>{save.isPending?"Salvando...":"Salvar configuração da base"}</button><button type="button" style={secondary} onClick={()=>{setEditing(undefined);setBaseOmieId("");setAdvanceEnabled(false);}}>Cancelar</button></div>
    </form>}
    <div style={card}><table style={{width:"100%"}}><thead><tr><th align="left">Base Omie</th><th align="left">Etapa de envio</th><th align="left">Etapa de erro</th><th align="left">Etapa de sucesso</th><th align="left">Adiantamento</th><th align="left">Status</th><th align="left">Ações</th></tr></thead><tbody>{rows.map(item=><tr key={item._id}><td>{item.baseOmieId?.nome||"-"}</td><td>{item.etapaEnvio}</td><td>{item.etapaErro}</td><td>{item.etapaSucesso}</td><td>{item.gerarAdiantamento?`Sim · ${item.contaCorrenteAdiantamentoId?.descricao||"conta"} · ${item.categoriaAdiantamentoId?.descricao||"categoria"}`:"Não"}</td><td>{item.status}</td><td><button style={secondary} onClick={()=>beginEdit(item)}>Alterar</button> <button style={danger} onClick={()=>confirm("Excluir as etapas desta base?")&&remove.mutate(item._id)}>Excluir</button></td></tr>)}{rows.length===0&&<tr><td colSpan={7} style={{padding:24,color:"#64748b"}}>Nenhuma base configurada para este gatilho.</td></tr>}</tbody></table></div></>}
  </div>;
}
function ConfiguracoesPage(){
  const {http}=useOonApi();const query=useCatalogs();const cache=useQueryClient();const [tab,setTab]=useState<"template"|"emails"|"automacoes">("template");const [baseFilter,setBaseFilter]=useState("");const [editing,setEditing]=useState<Item|null>();const [message,setMessage]=useState("");const [error,setError]=useState("");const [internalEmails,setInternalEmails]=useState("");const [enabledAutomations,setEnabledAutomations]=useState<string[]>([]);
  const automationDefinitions=[["automacao-aprovacao-automatica","Aprovação automática","Gera a fatura assim que o webhook criar o processo."],["automacao-revisao-automatica","Revisão automática","Aprova a fatura gerada e anexa o PDF no Omie."],["automacao-envio-automatico","Envio automático","Envia o e-mail e conclui a OS sem confirmação manual."],["automacao-reprocessar-falha","Reprocessar falha automático","Tenta novamente falhas transitórias, com limite de segurança."]] as const;
  const automationCodes=new Set(automationDefinitions.map(item=>item[0]));
  ...
  useEffect(()=>{setEnabledAutomations(automationDefinitions.filter(([code])=>(query.data?.configuracoes||[]).some(item=>item.codigo===code&&item.status==="ativo"&&String(item.valor).toLowerCase()==="true")).map(([code])=>code));},[query.data?.configuracoes]);
  const finish=(text:string)=>{setMessage(text);setError("");setEditing(undefined);cache.invalidateQueries({queryKey:["doc-custom-catalogs"]});};
  ...
  const saveAutomations=useMutation({mutationFn:async()=>(await http.put("/api/doc-custom/configuracoes/automacoes",{habilitadas:enabledAutomations})).data,onSuccess:data=>{setError("");setMessage(data.message);cache.invalidateQueries({queryKey:["doc-custom-catalogs"]});},onError:e=>{setMessage("");setError(errorText(e));}});
  ...
  const rows=(query.data?.configuracoes||[]).filter(c=>c.codigo!=="email-destinatarios-internos"&&!automationCodes.has(c.codigo as any)&&(!baseFilter||String(c.baseOmieId?._id||c.baseOmieId||"")===baseFilter));
  const tabs=[["template","Dados complementar do template"],["emails","E-mails Internos"],["automacoes","Automações"]] as const;
  return <div><Header title="Configurações" description="Organize dados do template, destinatários internos e automações da esteira."/><Notice message={message} error={error}/>
  <nav aria-label="Abas de configurações" style={{display:"flex",gap:8,borderBottom:"1px solid #cbd5e1",marginBottom:18}}>{tabs.map(([id,label])=><button key={id} type="button" onClick={()=>{setTab(id);setMessage("");setError("");}} style={{...secondary,borderRadius:"8px 8px 0 0",borderBottom:tab===id?"3px solid #0284c7":"3px solid transparent",fontWeight:tab===id?700:500}}>{label}</button>)}</nav>
  {tab==="emails"&&<section style={{...card,borderColor:"#7dd3fc",background:"linear-gradient(135deg,#f0f9ff,#fff)"}}>
    <h2 style={{margin:"0 0 5px"}}>E-mails Internos</h2><p style={{margin:"0 0 12px",color:"#475569"}}><strong>Cópias internas das faturas:</strong> estes endereços serão incluídos automaticamente em <strong>Cc</strong> em toda fatura enviada ao cliente.</p>
    <div style={{display:"grid",gridTemplateColumns:"minmax(280px,1fr) auto",gap:12,alignItems:"end"}}><label style={{display:"grid",gap:6,fontWeight:600}}><span>E-mails internos</span><textarea aria-label="E-mails internos das faturas" style={{...input,minHeight:92,resize:"vertical",fontFamily:"inherit",fontWeight:400}} value={internalEmails} onChange={event=>setInternalEmails(event.target.value)} placeholder={"financeiro@empresa.com.br\nfaturamento@empresa.com.br"}/><small style={{color:"#64748b",fontWeight:400}}>Separe por linha, vírgula ou ponto e vírgula.</small></label><button type="button" style={button} disabled={saveInternal.isPending||!internalEmails.trim()} onClick={()=>saveInternal.mutate()}>{saveInternal.isPending?"Salvando...":"Salvar e-mails internos"}</button></div>
  </section>}
  {tab==="automacoes"&&<section style={card}><h2 style={{marginTop:0}}>Automações da esteira</h2><p style={{color:"#475569"}}>Ative somente as decisões que não precisam de intervenção humana. As etapas continuam auditadas.</p><div style={{display:"grid",gap:10,margin:"18px 0"}}>{automationDefinitions.map(([code,label,description])=><label key={code} style={{display:"grid",gridTemplateColumns:"auto 1fr",gap:"2px 10px",padding:12,border:"1px solid #e2e8f0",borderRadius:9}}><input type="checkbox" style={{gridRow:"1/3"}} checked={enabledAutomations.includes(code)} onChange={event=>setEnabledAutomations(current=>event.target.checked?[...current,code]:current.filter(item=>item!==code))}/><strong>{label}</strong><span style={{color:"#64748b"}}>{description}</span></label>)}</div><button type="button" style={button} disabled={saveAutomations.isPending} onClick={()=>saveAutomations.mutate()}>{saveAutomations.isPending?"Salvando...":"Salvar automações"}</button></section>}
  {tab==="template"&&<><div style={{display:"flex",justifyContent:"space-between",marginBottom:12}}><select style={{...input,maxWidth:320}} value={baseFilter} onChange={e=>setBaseFilter(e.target.value)}><option value="">Todas as bases Omie</option>{query.data?.bases.map(b=><option key={b._id} value={b._id}>{b.nome}</option>)}</select><button style={button} onClick={()=>setEditing({} as Item)}>+ Novo dado complementar</button></div>
  ...
  <div style={card}><table style={{width:"100%"}}><thead><tr><th align="left">Código</th><th align="left">Descrição</th><th align="left">Tipo</th><th align="left">Valor</th><th align="left">Base Omie</th><th align="left">Status</th><th align="left">Ações</th></tr></thead><tbody>{rows.map(c=><tr key={c._id}><td>{c.codigo}</td><td>{c.descricao}</td><td>{c.tipo}</td><td>{c.valor}</td><td>{c.baseOmieId?.nome||"Todas"}</td><td>{c.status}</td><td><button style={secondary} onClick={()=>setEditing(c)}>Alterar</button> <button style={danger} onClick={()=>confirm("Excluir esta configuração?")&&remove.mutate(c._id)}>Excluir</button></td></tr>)}{rows.length===0&&<tr><td colSpan={7} style={{padding:24,color:"#64748b"}}>Nenhum dado complementar cadastrado.</td></tr>}</tbody></table></div></>}
  </div>;
}
